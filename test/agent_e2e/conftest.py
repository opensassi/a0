"""Agent E2E test infrastructure: TuiDriver and shared fixtures."""
...
MOCK_SERVER = str(Path(__file__).resolve().parent.parent / "e2e" / "mock_deepseek_server.py")
...
class TuiDriver:
    ...
    def start(self):
        ...
        if self.pid == 0:
            ...
            os.close(slave_fd)
            # Give the child time to exec a0 and start up
            time.sleep(1.0)
    ...
    def send_enter(self):
        """Send Enter (LF, matches FTXUI Event::Return)."""
        self._write_raw(b"\n")
    ...
    def send_paste(self, text):
        """Send text via bracketed paste protocol (ESC[200~ ... ESC[201~).

        This simulates a terminal-emulator paste. The content between the
        markers may include newlines which FTXUI should treat as literal
        text rather than Enter/submit.
        """
    ...
    def capture(self, timeout=2.0):
        """Read available rendered output, strip ANSI, return plain text."""
        output = b""
        deadline = time.monotonic() + timeout
        while time.monotonic() < deadline:
...
class AgentSubprocess:
    """Runs a0 as a subprocess (non-TUI mode) with assertions."""
    ...
    def run(self, goal, timeout=30):
        ...
        # Global flags MUST come before the subcommand in CLI11
        cmd = [
            A0_BIN,
            "--a0-dir", self.a0_dir,
            "--skills-dir", SKILLS_DIR,
    ...
    def stderr_empty(self):
        return self.result and len(self.result.stderr) == 0
